...
class ForecastingController extends Controller
{
    ...
    {
        $records = ProcessedRecord::query()->orderBy('year')->orderBy('month')->get();
        abort_if($records->count() < 6, 422, 'At least 6 processed records are required for forecasting.');

        Storage::makeDirectory('ml');
        $headers = ['year', 'month', 'consumption_kwh', 'temperature', 'humidity', 'rainfall', 'solar_irradiance', 'peak_demand_kw', 'lag_1', 'lag_2', 'trend', 'month_sin', 'month_cos'];
        $lines = [implode(',', $headers)];

        foreach ($records as $record) {
            $lines[] = implode(',', array_map(fn ($field) => $record->{$field}, $headers));
        }
    ...
    {
        $python = file_exists(base_path('venv314/Scripts/python.exe')) ? base_path('venv314/Scripts/python.exe') : 'python';
